...
- [x] `lessonSources`: telegram primary row always; a youtube row **only when the lesson's own title or caption contains the link**. The plan expected trailing YouTube links to confirm a grouping and they cannot: of @doros_alkulify's 3,977 bare-link messages 1,417 sit between two other links and only 648 are followed by audio, because the links are a separate republication stream posted hours to days later in batches. Position cannot bind a link to a lesson; the rest wait for the v1 YT↔TG map (§8)
