...
Linux and Python 3.10 through 3.13 are release-tested. The Apple MPS device path is validated on hardware, on an M3 with 24 GB unified memory; lower-memory and other M-series systems are compatibility targets, not release-tested. Its FP16 output matched CPU FP32 byte for byte on a local 8-clip corpus, measured against the ungated model mirror rather than the gated default checkpoint, and batch sizing stays static at the MPS default of 8 because the adaptive controller reads memory only on CUDA. `docs/performance.md` carries the measurements and their limits. ROCm has no dedicated CLI mode and is experimental through PyTorch's CUDA-compatible interface. Multi-GPU scheduling is not provided; one command selects one compute device.
