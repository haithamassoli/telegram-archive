...
- [ ] Playback gate (v2.5 amendment, 2026-09-11 — supersedes the plan §Phase-0 line "the source files ship as-is"): **re-encoding is allowed again.** The ban was never about codecs, it was about merging, and merging is gone; nothing about a virtual timeline forbids transcoding a single part. So the gate is now two questions, not one:
  1. Which of the real extensions in `mediaObjects.ext` (ogg, mpeg, m4a, mp4, wma, amr, wav, opus) actually play, seek and honour Range on iOS Safari, Android Chrome, desktop — tested on real devices, `decision` + `testedOn` into `m0.gates.json`
  2. What the failures get transcoded to. Default: AAC in mp4 (`.m4a`), the one format no browser argues with. Not Opus — iOS is exactly the platform this gate exists for
  **Three laws the transcode obeys, or it breaks identity:** the re-encoded file is a *derived artifact*, never a `mediaObjects` row — the original sha256 stays the transcript identity and the `lessonParts` member. Key it deterministically at `playback/{sha256}.m4a` so the signed-URL endpoint can pick it by extension with no schema field and no new column in the site repo's Convex schema. And transcode only what fails step 1: a format the browsers already play gets re-encoded never. ponytail: extension-based routing in the URL endpoint, add a `playbackKey` field if the exception set ever stops being a handful of extensions
  **Download is not a requirement.** Every lesson carries its `telegramUrl`, and Telegram serves the original file better than we would. The player only has to play
...
- [x] Scope note surfaced by the first full scan: 9,793 of the 11,460 unique binaries are audio; the other 1,667 are 1,212 images, 338 PDFs, 94 videos (6.2 h — `video/mp4` and one `3gpp`), 18 PNGs and 4 DOCX. The plan scopes M2 to "unique audio sha256s", so the videos are excluded and reported rather than silently dropped. **Decided (2026-09-11, owner): the videos are never transcribed.** Plan Open Item 4 closes as "no" against its own "yes" recommendation. M2's 100% exit stands as measured — it was always counted over audio binaries — and nothing downstream indexes video: no chunks, no lessons, no search documents. Revisiting costs ~40 min of GPU plus a partial reindex, and no more than that.
...
- [ ] Grouping v3 (`group-v3`): 5,284 lessons, 423 of them named by a **series header** and 177 duplicate rows retired. Two things the 2017-and-after corpus needs and nothing else. **Series headers:** @doros_alkulify opened in 2017 by uploading its back catalogue in bulk — one post naming a book, then the whole book as `01.MP3`, `02.MP3`, `03.MP3`. `filename_title` rejects those names and is right to, so the series existed nowhere but the header, and 406 of the channel's 519 untitled lessons were exactly this. A text post with no media that names a book — a bracketed name, or a declared `عدد الدروس` — and has a run of at least two files behind it now names every lesson in that run as «SERIES (N)», with N from the file's own number so a deleted episode does not renumber the book. The channel validates the rule with its own arithmetic: 22 of its 38 declared counts match the run behind them file for file, 8 more within two. Both guards were needed — «الدروس بصيغة أخرى 👇» (41 of them, a re-upload announcement) names no book, and «تعليق على الجامع [ 173 ]» brackets an episode number, not a series. Result: untitled 519 → 9 on @doros_alkulify, 29 series with contiguous 1..N numbering, `needs_review` 1,456 → 933, @alkulife untouched (0 headers, 24 untitled, same 1,851 lessons). **The same lesson posted twice** — the screenshot case: a live stream's recording goes up and the second copy follows minutes later. Two shapes, and only one of them is a heuristic. Byte-identical, under a machine name (`4_5906797496…m4a`): 186 messages in @doros_alkulify carry a sha256 an earlier message already carried, 0 in @alkulife, and M1's deduplication makes that a proof rather than a guess. Re-encoded, an `.m4a` and then the same lesson as `.mp3`: 27 of those, matched on length within 2 s **and** a name that survives re-spacing, re-bracketing and the other digit alphabet, **and** a 6 h window. All three guards earn their place — length alone merges episodes 3 and 4 of one 2017 book, which run 3,951 s and 3,950 s under the names `03.MP3` and `04.MP3`; the name alone merges 64 unrelated clips, because «تتمة مهمة» is a caption this channel has reused for nine years. 22 of the 27 pairs are identical to the millisecond. In both shapes the first upload is kept and the second composes nothing. **Neither change moves a `lessonKey`** — the dry run reports 0 new keys, 423 retitles, 177 retired rows and 0 approved or hand-edited rows among them. Binary coverage goes 9,705 → 9,678: the 27 that leave are the duplicate encodings, and every retired row's audio is still played by the lesson that absorbed it. Their transcripts stay in R2, orphaned and reported by `reconcile-artifacts`, never deleted
...
- [ ] Relevance baseline: replay v1 query logs against v1 and v2; record results (§8.4). **Do the v1 half first and freeze it to a file.** v1's Meilisearch is scheduled for shutdown (§7 of `next-decisions.md`), and this milestone's exit criterion is a *comparison* — once that instance is off, v1's side of it is unmeasurable and M4 can never exit. Replaying the 66 `eval-questions` + the query log against v1 once and storing the answers under `legacy/assoli-v1/` costs an hour and unblocks the shutdown permanently
